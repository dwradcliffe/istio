...
func ValidateServiceEntry(name, namespace string, config proto.Message) (errs error) {
	...
	if len(serviceEntry.Hosts) > 1 && len(serviceEntry.Addresses) > 0 {
		errs = appendErrors(errs, fmt.Errorf("service entry must have exactly one host when address is present"))
	}
